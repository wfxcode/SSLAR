...
{
  {
    hvg=9999999; min_cont=0.07; numiters=100; lam=100; nmf = "ssNMF"; regr = "lar"; regr_type = "nnls"
  }
  for(data_name in data_name_list){
    ...
    # change 1 and 1.1
    format = "cl_n=%s/"
    folder = sprintf(format, substr(paste(cl_n_list, collapse = "_"), 1 , 20))

    save_path = paste(ajust_path, folder, sep = "")

    # change 2
    save_files = get_save_files(path = save_path, params_array = cl_n_list)
...

    {
      ...
      params = list()
      # change 3
      for(i in seq_len(length(cl_n_list))){
        # change 4 and 5
        cl_n = cl_n_list[i]
