...
{cl_n=9999999; hvg=9999999; min_cont=0.07; numiters=100
...
for(data_name in data_name_list){
  ...
  # change 1 and 1.1
  format = "lam=%s/"
  folder = sprintf(format, substr(paste(lam_list, collapse = "_"), 1 , 20))

  save_path = paste(ajust_path, folder, sep = "")

  # change 2
  save_files = get_save_files(path = save_path, params_array = lam_list)
  ...
  params = list()
  # change 3
  for(i in seq_len(length(lam_list))){
    # change 4 and 5
    lam = lam_list[i]
