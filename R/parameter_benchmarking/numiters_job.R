...
{cl_n="all"; hvg="all"; min_cont=0.00; lam=100}
for(data_name in data_name_list){
  ...
  # change 1 and 1.1
  format = "numiters=%s/"
  folder = sprintf(format, substr(paste(numiters_list, collapse = "_"), 1 , 20))

  save_path = paste(ajust_path, folder, sep = "")

  # change 2
  save_files = get_save_files(path = save_path, params_array = numiters_list)
...

  {
    ...
    params = list()
    # change 3
    for(i in seq_len(length(numiters_list))){
      # change 4 and 5
      numiters = numiters_list[i]
