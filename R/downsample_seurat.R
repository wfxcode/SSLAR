...
downsample_seurat <- function(se_obj,
                              clust_vr,
                              cluster_markers,
                              cl_n = 100,
                              hvg = 3000) {
  ...
  if (!(cl_n == "all" || is.numeric(cl_n))) stop("ERROR: cl_n must be an object of class integer!")
  if (! (is.numeric(hvg) || hvg == "all")) stop("ERROR: hvg must be an object of class integer or a string 'uns'!")
