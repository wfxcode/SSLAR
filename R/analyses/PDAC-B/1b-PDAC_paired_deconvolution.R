...
# ST data
{
  st_tibble = read.csv(file = "R/analyses/data/PDAC/PDAC-B-ST1-filtered.csv",header = TRUE,row.names=1)
  # st_position = read.csv(file = "R/analyses/data/PDAC/PDAC-B-ST1_positions.csv",header = TRUE,row.names=1)
  barcodes <- utils::read.csv("R/analyses/data/PDAC/PDAC-B-ST1_positions.csv", sep = "\t", header = FALSE)

  st_se = Spaniel::createSeurat(counts = st_tibble,
                                barcodeFile = ("R/analyses/data/PDAC/PDAC-B-ST1_positions.csv"),
                                projectName = "PDAC-B",
                                sectionNumber = "1")
  st_se <- Seurat::ScaleData(st_se)
  st_se = SCTransform(st_se)
  image <- Spaniel::parseImage("R/analyses/data/PDAC/PDAC-B-ST1.jpg")
  st_se@images <- list(image)
  saveRDS(object = st_se,
          file = sprintf("R/analyses/data/PDAC/PDAC-B_ST.RDS"))
  st_se = readRDS("R/analyses/data/PDAC/PDAC-B_ST.RDS")
}
## Spatial Deconvolution
Seurat::Idents(object = indrop_pdac_b) <- indrop_pdac_b@meta.data[, clust_vr]
cluster_markers_a <- Seurat::FindAllMarkers(object = indrop_pdac_b,
                                            verbose = TRUE,
                                            only.pos = TRUE,
                                            assay = "SCT",
                                            slot = "data")
saveRDS(object = cluster_markers_a,
        file = "R/analyses/data/PDAC/PDAC_B_markers.rds")
cluster_markers_a <- readRDS("R/analyses/data/PDAC/PDAC_B_markers.rds")
...
#                                        perl = TRUE)
indrop_pdac_b_down <- downsample_se_obj(se_obj = indrop_pdac_b, clust_vr = clust_vr,
...
decon_mtrx <- mixture_deconvolution_nmf1(nmf_mod = nmf_mod,
                                         ...
                                         min_cont = 0.01)
saveRDS(decon_mtrx,"R/analyses/data/PDAC/decon_mtrx_pdac_b.RDS")
...

{
  ...
  scatterpie_plt <- ggplot() +
    ...
    labs(title = "PDAC-B Spatial scatterpie",
         color = "Cell types") +
    theme(
  ...
  scatterpie_plt
  save_plot("R/analyses/img/PDAC-B/PDAC_all.jpg",scatterpie_plt,
