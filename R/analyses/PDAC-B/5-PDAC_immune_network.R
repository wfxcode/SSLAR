...
}
{
  ...
  cell_types_plt <- sort(unique(pdac_B$annotation))
  decon_mtrx_pdac_b = readRDS("R/analyses/data/PDAC/decon_mtrx_pdac_b.RDS")

  pdac_plt_names <- pdac_B@meta.data%>%
  ...
  decon_mtrx <- decon_mtrx_pdac_b[, colnames(decon_mtrx_pdac_b)[!colnames(decon_mtrx_pdac_b) %in% "res_ss"]]
  ...
  plot(graph_ntw,
       ...
       main = "PDAC-B spatial interaction network")
}
...
decon_mtrx <- decon_mtrx_immune[, colnames(decon_mtrx_immune)[!colnames(decon_mtrx_immune) %in% "res_ss"]]
{
  ...
  plot(graph_ntw,
       ...
       vertex.label.family = "Helvetica", # Font family of the label (e.g.“Times”, “Helvetica”)
       layout = layout.circle,
       main = "PDAC-B immune spatial interaction network")
}
