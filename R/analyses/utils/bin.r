...
joint_scatterpie_fun <- function(se_obj, cell_types_all, img_path1, img_path2,
  ...
  #         legend.key.size = unit(1.9, "line")) +
  #   guides(fill = guide_legend(nrow = 5, override.aes = list(size = 0.1)))
  # # cowplot::get_plot_component()
  legend_gtable <- get_legend(anterior_plt)
  anterior_plt = anterior_plt+theme(legend.position = "none")
  # browser()
  scatterpie_tmp <- cowplot::plot_grid(plotlist = list(anterior_plt,NULL,
                                                       posterior_plt),
                                       nrow = 1,
                                       rel_widths = c(1,-0.1, 1))

  if (arr_leg) {
    scatterpie_tmp <- cowplot::plot_grid(plotlist = list(NULL,scatterpie_tmp,NULL,
                                                         legend_gtable),
                                         ncol = 1,
                                         rel_heights = c(-0.2,1,-0.2, 0.15),
                                         align = "v")+
      theme(plot.background = element_rect(fill = "white"))
  }

  # save_plot("tmp.jpg",tmp,
  #           base_width = 10,
  #           base_height = 6)
...
join_spatial_plots_2 <- function(spatial_obj, feat, nam) {
  ###########################################################################################################
  #### This functions takes in a feature to plot in 2 spatial slides and returns it with a unique legend ####
  ###########################################################################################################
  library(ggplot2)
