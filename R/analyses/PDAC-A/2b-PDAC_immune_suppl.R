...
{
  decon_mtrx = readRDS("R/analyses/data/PDAC/decon_mtrx_ica_pdac_a.RDS")
  ...
  scatterpie_plt <- ggplot() +
    ...
    labs(title = "PDAC-A Immune Spatial scatterpie",
         color = "Cell types") +
    theme(
  ...
  save_plot("R/analyses/img/PDAC/PDAC_immune_all.jpg",scatterpie_plt,
...
}
{
  ...
  ct_plt <- lapply(cell_types, function(ct){
    ...
    #                                 # sample_dict[[geo]],
    #                                 dfa_col[dfa_col$df_name == ct, "plt_name"])) +
  ...
  immune_ct_plt = ggpubr::ggarrange(plotlist = ct_plt,
                                    ...
                                    nrow = 2,
                                    align = "hv")
  save_plot("R/analyses/img/PDAC/immune_all_types/all_types.pdf",immune_ct_plt,bg="white",
            base_width = 24,
            base_height = 9)
  i=1
  for (ct in cell_types) {
    path = paste( "R/analyses/img/PDAC/immune_all_types/",ct,".jpg", sep = "")
    print(path)
